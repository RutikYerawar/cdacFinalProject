...
export default function Itemsize() {
  ...
  const getItemSize = () => {
    ...
    const url = `http://localhost:7071/itemSize/${itemid}`;
  ...
  const getToppings = () => {
    const url = `http://localhost:7071/toppings/showAll`;
    axios.get(url).then((response) => {
      const result = response.data;
      console.log(result);
      if (result.status === "success") {
  ...
  const add2db = (varioussize) => {
    ...
    if (toppingId === "") {
      const url = `http://localhost:7071/cart/addWithoutToppings`;
      ...
      const url = `http://localhost:7071/cart/addWithToppings`;
